...
| `src/supportcover_rag/sensitivity.py`, `src/supportcover_rag/phase3.py` | KEEP | Development-only runner and protocol validation implemented | Reuses canonical packing algorithms for generator-free OFAT/MMR/ablation screening | Phase-3 development evidence | Dedicated Phase-3 tests cover role/source/hash/plan/output invariants | Execute only after the environment and smoke gates | Ready, execution pending |
| `src/supportcover_rag/freeze.py` | KEEP | Deterministic schema-v2 freeze helper exposed through `freeze-development` | Records selected settings, development/final split hashes, and hashed development evidence | Phase-3 absolute freeze point | Dedicated deterministic-freeze test | Run only after recorded development selection | Ready, execution pending |
...
| `configs/phase3_sensitivity.yaml` | KEEP | Corrected to HotpotQA train + frozen development IDs and consumed by Phase-3 CLI | Exact OFAT, MMR, and clean-ablation grids; no Cartesian product | Planned development evidence; no runs completed yet | Checked-in protocol test plus Phase-3 runner tests | Preserve final isolation and record shortlist/decision artifacts | Ready, execution pending |
