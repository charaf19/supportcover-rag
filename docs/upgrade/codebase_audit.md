...
- `tests/`: 53 collected tests covering the runner, output layout, prompting, split utilities, packing baselines, ablations, statistics, global retrieval, resource monitoring, summaries, and error analysis.
...
- No notebooks, scripts directory, CI configuration, local datasets, or local experiment outputs are present.
...
| `src/supportcover_rag/packing.py` | KEEP | Active canonical selectors | Contains all active selectors; no competing implementation found | Historical and planned packing behavior | Direct baseline, paragraph, and ablation tests | Keep canonical | Preserved |
...
| `src/supportcover_rag/external_baselines/` | KEEP | Interface active by dependency injection; no implementation installed | Boundary only, not a fabricated compressor | Required contemporary comparison | Runner checks return type; no real adapter test | Select one reproducible method later or document external blocker | Deferred |
...
| `configs/default.yaml` | DEPRECATE | Active default example | `ablations.variants` still uses conflated `no_coverage` while code defaults use separate factors | Captures early behavior | Config loads | Correct in Phase 2 or introduce a new canonical config; preserve hash in inventory | Deferred to avoid mixing phases |
...
| `configs/final_*.yaml` | KEEP | Planned and blocked | Frozen numeric values are `null`; split and freeze files do not exist | No result significance yet | Config-load check only | Do not run until dev tuning, freeze, and asset gates pass | Preserved unchanged |
